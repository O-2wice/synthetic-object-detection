...
assert len(nb.cells)==58 and len(cells)==31
...
for class_id in range(3):
    ...
    assert metrics=={'true_positives':1,'total_preds':1,'total_trues':1},metrics
...
# Verify the original architecture and loss against the original notebook when available.
original=ROOT.parent/'HW1_Image_Object detection_2 (3).ipynb'
if original.exists():
    old=nbformat.read(original,as_version=4)
    for index,name in [(34,'CustomObjectDetectionModel'),(41,'CustomObjectDetectionModel'),(33,'CompositeLoss')]:
        source=old.cells[index].source.replace('pretrained=True','weights=models.ResNet18_Weights.DEFAULT',1)
        if index!=33: source=source.replace('pretrained=True','weights=models.VGG16_Weights.DEFAULT',1)
        old_node=next(n for n in ast.parse(transformer.transform_cell(source)).body if isinstance(n,ast.ClassDef) and n.name==name)
        new_node=next(n for n in ast.parse(cells[index]).body if isinstance(n,ast.ClassDef) and n.name==name)
        assert ast.dump(old_node)==ast.dump(new_node),(index,name)
